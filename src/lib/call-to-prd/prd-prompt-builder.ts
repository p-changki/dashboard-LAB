...
- 시퀀스 다이어그램 바로 뒤에는 별도의 \`\`\`mermaid fenced code block으로 \`flowchart TD\` 기능 흐름도도 추가
- 기능 흐름도에는 시작, 주요 결정 분기, 결과 상태를 포함
- 화면/운영/API 전환 지점은 간결한 노드로 표현하고, 세부 설명은 본문 bullet로 보완
...
- 추측은 "추정:" 접두사
- 시퀀스 다이어그램과 기능 흐름도는 mermaid 문법이 깨지지 않도록 코드블록만 출력
