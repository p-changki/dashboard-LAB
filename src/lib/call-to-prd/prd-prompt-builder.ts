...
export function buildCallToPrdPrompt(options: PrdPromptOptions): string {
  const { transcript, projectName, projectContext, baselineTitle, baselinePrd, customerName, additionalContext, intake, pdfAnalysis, pdfFileName } = options;
...
아래 내용은 현재 로컬 프로젝트에서 추출한 요약입니다. 기존 구조와 운영 맥락을 반영해 추가 기능 문서를 작성해주세요.
