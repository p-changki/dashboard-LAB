...
| 10-3.3 | ~~`upload/route.ts:279,300,424,483,625,718,759,792,928,...` 호출부 교체~~ → **(구현 시 정정)** 10-3.1 전수 조사 결과 이 추정은 과대했다. 45개 한국어 문자열 중 **미번역 사용자 노출은 2곳뿐** (`:290` PDF 분석 진행률, `:317` 프로젝트 컨텍스트 실패). 나머지는 `warningMap` 완전일치(12) · 정규식 분기(4) · `includes()` 분기(2) · en/ko 삼항(20)으로 이미 처리됨. AI 프롬프트 내부(5)는 유지 대상 |
...
- **구현 진행 상태 체크리스트**:
  ...
  - [x] 본 설계 문서 + 구현 파일 git 커밋 — PR #3 머지 완료 (`bf27bbd`)

---

## 10. Phase 10-3 구현 기록 (완료)

**범위 정정**: 10-3.1 전수 조사로 10-3.3의 추정(약 10곳)이 과대함을 확인. 실제 미번역 2곳만 교체. 대신 설계 범위 밖이던 `project-context.ts`에서 결함 2건을 발견해 함께 처리.

| Task | 결과 |
|---|---|
| 10-3.1 | `upload/route.ts` 한국어 45건 분류 완료 (위 10-3.3 행 참조) |
| 10-3.2 | `messages.ts`에 `formatCallToPrdPdfAnalysisProgress`, `formatCallToPrdProjectContextFailed` 추가 |
| 10-3.3 | `:290`, `:317` 2곳 교체 |
| 10-3.4 | `working-context.ts`에 `buildOriginalCallContext` 신설, `formatSourceList` 재사용. `buildCallWorkingContext`(compact)와 독립 유지 |
| **10-3.5 (신규)** | `project-context.ts`의 사용자 노출 에러 4종이 en locale에서 한국어 반환 → `getCallToPrdProjectContextIssueMessage(issue, locale)` + `inspectLocalProjectContext(path, locale)` 파라미터 추가. 호출부 3곳 갱신 |
| **10-3.6 (신규)** | 10-3.5 수정 후 드러난 **캐시 오염** — 프로젝트 컨텍스트 캐시가 지역화 문자열을 저장해 ko 요청 뒤 en 요청이 한국어를 받음. 캐시에 `issue` 코드를 저장하고 요청마다 지역화하도록 `CachedProjectContextResult` 분리 |

**검증**
- lint → build → type-check 통과
- **프롬프트 동등성**: `buildOriginalCallContext` vs 원본 인라인 로직, 288개 입력 조합 비교 → 불일치 0 (AI 프롬프트라 바이트 단위 동일성 필수)
- **locale 교차**: ko→en→ko→en 요청으로 캐시 오염 재현 시도 → 각각 올바른 언어
- **실제 파이프라인 2회**: 단일 모드(문서 2건, 경고 0) / **dual 머지 모드**(`merging` 진입, diffReport 정상 생성 = 추출 빌더 실행 확인)

**미해결 (기록만)**
- `:317` `formatCallToPrdProjectContextFailed`는 예기치 않은 throw 시에만 도달하는 폴백이라 실행 검증 불가
- 프로젝트 컨텍스트 실패 헬퍼가 구체(4종)/일반(1종) 2개로 중복 — 신규 사유 추가 시 배치 기준 모호
