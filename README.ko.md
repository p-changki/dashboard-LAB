...
`dashboard-LAB`은 내가 가진 AI CLI 도구를 중심으로 프로젝트, 문서, 고객 응대, 회의 운영, 일일 업데이트, 시그널 기반 글쓰기, PRD 워크플로를 하나의 데스크톱 워크스페이스에서 다루게 해주는 로컬 퍼스트 앱입니다.
...
- 매일 들어오는 시그널을 바탕으로 초안, 조사, 팩트체크까지 한 흐름에서 처리할 수 있습니다
...
- `Meeting Hub`: 팀, 회의 기록, 액션 아이템을 로컬 프로젝트 문맥과 함께 관리
- `Call → PRD`: 통화, 메모, 이슈 내용을 PRD와 역할별 후속 문서로 전환
- `Info Hub`: 뉴스, 트렌드, 패키지 업데이트, 보안 신호를 매일 확인
- `Signal Writer`: 매일 모인 시그널을 소셜 초안, 앵글 보드, 보조 리서치로 연결
...
2. `dashboard-LAB-0.1.14-arm64.dmg` 또는 `dashboard-LAB-0.1.14-arm64-mac.zip` 같은 macOS 설치 파일을 내려받습니다.
...
Arch Linux 및 Arch 기반 배포판(CachyOS 등) 사용자는 [AUR](https://aur.archlinux.org/packages/dashboard-lab-bin)을 통해 미리 빌드된 패키지를 설치할 수 있습니다:
...
그 외 Linux 배포판이거나 Windows 환경일 경우 다음 명령어를 사용합니다:
...
1. [Releases](https://github.com/p-changki/dashboard-LAB/releases) 페이지를 엽니다.
2. `dashboard-LAB-0.1.14-arm64.dmg` 같은 macOS 설치 파일을 내려받습니다.
3. 내려받은 앱 패키지를 열어 실행합니다.
